...
def logEpoch(logFileName, epoch, loss_train, loss_val, f1_train, f1_val, aveAcc_train, aveAcc_val, overallAcc_train, overallAcc_val, f1_classes_train, f1_classes_val, acc_classes_train, acc_classes_val):
	...
	logFile.close()
#============================== Minibatch training =============================
def trainSegmentation(train_loader, model, criterion, optimizer, epoch, n_Epochs, numOfIterPerEpoch_train, nClasses, GPU, oneHot,useWeightedLoss):
	...
	start = timer()
	for iteration, (data, labels, weights) in enumerate(train_loader):
		data = Variable(data)
		...
		outputs = model(data)
		if useWeightedLoss:
			loss_minibatch = criterion(outputs, labels, weights)
		else:
...
def valSegmentation(val_loader, model, criterion, epoch, n_Epochs, numOfIterPerEpoch_val, nClasses, GPU, oneHot, useWeightedLoss):
	...
	f1_classes  = [0 for i in range(nClasses)]
	acc_classes = [0 for i in range(nClasses)]  
	for iteration, (data, labels, weights) in enumerate(val_loader):
...
#========Stitching Patches in Test Areas========================================================================================================
def stitchPatches(networkOut_allClasses,num_patches_x,num_patches_y,transposiotn):
	print('stitching patches')
	nClasses = networkOut_allClasses.size(1)
	patchSize =  networkOut_allClasses.size(2)
	output_allClasses = torch.Tensor(nClasses,patchSize + transposiotn*(num_patches_x-1), patchSize + transposiotn*(num_patches_y-1))
	for cls in range(nClasses):
		...
		output = torch.Tensor(patchSize + transposiotn*(num_patches_x-1), patchSize + transposiotn*(num_patches_y-1))
		...
		for i in range(networkOut.size(0)):
			for j in range(networkOut.size(1)):
				...
				if i == networkOut.size(0)-1:
					oe_1 = i*transposiotn + patchSize 
					ne_1 = patchSize 	


				if j == networkOut.size(1)-1:
					oe_2 = j*transposiotn + patchSize
					ne_2 = patchSize 	
				...
				if i < networkOut.size(0)-1 :
					if j == 0:
						...
						output[(i+1)*transposiotn:i*transposiotn + patchSize,patchSize + (j-1)*transposiotn:j*transposiotn + patchSize] = networkOut[i, j, transposiotn:patchSize, patchSize - transposiotn:patchSize] * d_1.transpose(0,1) +  networkOut[i+1,j,:patchSize-transposiotn,patchSize-transposiotn:patchSize] * a_1.transpose(0,1)
					else:
				...
				if j < networkOut.size(1)-1 :
					if i == 0:
						...
						output [patchSize + (i-1)*transposiotn:i*transposiotn + patchSize,(j+1)*transposiotn:j*transposiotn + patchSize] = networkOut[i,j,patchSize -transposiotn:patchSize,transposiotn:patchSize] * d_1 +  networkOut[i,j+1,patchSize -transposiotn:patchSize,:patchSize-transposiotn] * a_1
					else:
				...
				if i < networkOut.size(0)-1 and j < networkOut.size(1)-1 :
					output [(i+1)*transposiotn : patchSize + i*transposiotn,(j+1)*transposiotn:patchSize + j*transposiotn] = networkOut[i,j,transposiotn:patchSize,transposiotn:patchSize] * d_2d + networkOut[i+1,j+1,:patchSize-transposiotn,:patchSize-transposiotn] * a_2d 
...
def scores_test(output_allClasses,area,experiment,minus=0):
	...
	output = output.squeeze()
	print('data/fullMask_'+area+'.mat')
	mask_allClasses = sio.loadmat('data/fullMask_'+area+'.mat')
...
def crf(prob,area):
	...
	image_path = ('data/top/top_mosaic_09cm_area%s.tif') % area
	dsm_path = ('data/dsm/dsm_09cm_matching_area%s.tif') % area
